/**
  ...
  PC Mill 50.
  modified by Travis Sewell
  $Revision: 41058 f3e197eabd1a694af6e9f74abc094fce89defd1e $
  $Date: 2016-06-06 10:24:07 $
  
  FORKID {AE2102AB-B86A-4aa7-8E9B-F0B6935D4E9F}
*/

description = "Emco PC Mill 50";
vendor = "Fanuc";
legal = "Copyright (C) 2012-2016 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 24000;

longDescription = "Milling post for PC Mill 50 Fanuc emulator.";
...
minimumChordLength = spatial(0.75, MM);
minimumCircularRadius = spatial(0.75, MM);
maximumCircularRadius = spatial(1000, MM);
minimumCircularSweep = toRad(0.01);
maximumCircularSweep = toRad(28.65);
allowHelicalMoves = true;
allowedCircularPlanes = (0 << PLANE_XY) | (0 << PLANE_ZX) | (0 << PLANE_YZ);// allow any circular motion
...
properties = {
  ...
  sequenceNumberStart: 5, // first sequence number
  ...
  waitForSpindle: 3, // number of seconds to wait for spindle to spin up
  waitForCoolant: 0 // number of seconds to wait for coolant
};
...
var dFormat = createFormat({prefix:"H", decimals:0});
//This machine requires an H+10 callout for it's diameter offset. Does not support a D call
var xyzFormat = createFormat({decimals:(unit == MM ? 3 : 3), trimLeadZero:false, forceDecimal:true});
...
function onOpen() {
 ...
 // }
  ...
  //if (properties.writeTools) {
   ...
   //       "D=" + xyzFormat.format(tool.diameter) + "10" +
...
//  }
  ...
  writeBlock(gFormat.format(0), gPlaneModal.format(17), gFormat.format(40), gFormat.format(80));
  writeBlock(gAbsIncModal.format(90), gFeedModeModal.format(94), gFormat.format(98));
...
function onSection() {
  ...
  if (insertToolCall || newWorkOffset || newWorkPlane) {
    ...
    if (properties.useG28) {
     ...
     //block deleted next 7 lines to not force out comments for what the tool does. If you programmed it, you would know what it did, no? STM 1/30/15
    //writeln("");
 ...
 // wcs
  ...
  if (workOffset > 0) {
    if (workOffset > 6) {
     ...
     if (properties.useM6) {
      ...
      mFormat.format(6), conditional(tool.description, formatComment(tool.description)));;
    } else {
  ...
  if (insertToolCall ||
    ...
    if (tool.spindleRPM > 2700) {
      warning(localize("Spindle speed exceeds maximum value."));
    }
    writeBlock(
      sOutput.format(tool.spindleRPM), mFormat.format(tool.clockwise ? 3 : 4)
    );
    if ((tool.spindleRPM > 2700) && properties.waitForSpindle) {
  ...
  //{
    ...
    //if (c) {
     // writeBlock(mFormat.format(c));
      //if (properties.waitForCoolant) {
        //onDwell(properties.waitForCoolant);
      //}
    //} else {
...
function getCommonCycle(x, y, z, r) {
  //forceXYZ();
  //Block Skipped prior line to not force out XY postions on the drill canned cycle line;
...
function onClose() {
  ...
  if (!machineConfiguration.hasHomePositionX() && !machineConfiguration.hasHomePositionY()) {
    if (properties.useG28) {
      writeBlock(gAbsIncModal.format(91),gFormat.format(28), "Y" + xyzFormat.format(0)); // return to home
    }
  } else {
    var homeX;
